...
import { FaUserPlus, FaTrash, FaUserGraduate, FaSearch, FaEye, FaTimes } from "react-icons/fa";
...
export default function TeacherDashboard() {
  ...
  const API_URL = process.env.NNODE_API || "http://localhost:3001/api";

  useEffect(() => {
    const savedStudents = JSON.parse(localStorage.getItem("myStudents") || "[]");
  ...
  const handleAddStudent = async (e) => {
    ...
    if (studentIdInput.length !== 6) {
      setMessage({ text: "يرجى إدخال آخر 6 خانات من الرقم التعريفي", type: "error" });
      return;
    }
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          ...
          <form onSubmit={handleAddStudent} className="flex gap-2 w-full md:w-auto">
            ...
            <button disabled={isLoading} className="bg-main text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-opacity-90 transition shadow-md">
              <FaUserPlus />
        ...
        {message.text && (
          <div className={`mb-4 p-3 rounded-lg text-center regular ${message.type === "success" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
            {message.text}
          </div>
        )}
                ...
                <tr key={student.id} className="border-b hover:bg-gray-50 transition-colors">
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-gray-100 relative overflow-hidden hidden sm:block">
                        <Image src={`/Images/${student.avatar || 'avatar-boy-1.png'}`} fill alt="avatar" className="object-cover" />
                      </div>
                      <div>
                        <p className="bold text-main">{student.name}</p>
                        <p className="text-xs font-mono text-secondary">#{student.id}</p>
                      </div>
                    </div>
                  </td>
                  <td className="p-4 text-sm hidden md:table-cell">{student.joinDate}</td>
                  ...
                  <td className="p-4 text-center">
                    <div className="flex justify-center gap-3">
                      <button onClick={() => setSelectedStudent(student)} className="text-blue-500 hover:bg-blue-50 p-2 rounded-full transition" title="عرض التفاصيل">
                        <FaEye />
                      </button>
                      <button onClick={() => removeStudent(student.id)} className="text-red-400 hover:bg-red-50 p-2 rounded-full transition" title="حذف">
                        <FaTrash />
                      </button>
                    </div>
                  </td>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white w-full max-w-sm rounded-3xl overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300">
            <div className="relative h-32 bg-main flex justify-center">
                <button onClick={() => setSelectedStudent(null)} className="absolute top-4 right-4 text-white/70 hover:text-white text-xl">
                    <FaTimes />
                </button>
                <div className="absolute -bottom-12 w-24 h-24 rounded-full border-4 border-white bg-gray-100 overflow-hidden shadow-md">
                    <Image src={`/Images/${selectedStudent.avatar || 'avatar-boy-1.png'}`} fill alt="avatar" className="object-cover" />
                </div>
            </div>
            
            <div className="pt-16 pb-8 px-6 text-center">
                <h2 className="text-xl bold text-main mb-1">{selectedStudent.name}</h2>
                <p className="text-secondary font-mono mb-6">رقم الهوية: #{selectedStudent.id}</p>
                
                <div className="grid grid-cols-2 gap-4 mb-6">
                    <div className="bg-gray-50 p-3 rounded-2xl">
                        <p className="text-[10px] text-gray-400 mb-1">تاريخ الانضمام</p>
                        <p className="text-sm semi">{selectedStudent.joinDate}</p>
                    </div>
                    <div className="bg-gray-50 p-3 rounded-2xl">
                        <p className="text-[10px] text-gray-400 mb-1">نسبة الإنجاز</p>
                        <p className="text-sm semi text-green-600">{selectedStudent.progress}</p>
                    </div>
                </div>
